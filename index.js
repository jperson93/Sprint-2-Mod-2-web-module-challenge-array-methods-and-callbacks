...
const finals2014 = fifaData.filter((team) => {
...
function getFinals(array) {
    const finalTeams = fifaData.filter((teams) => {
...
function getYears(array,getFinalsCB) {
    return getFinalsCB(array).map(teams => teams.Year);
}
...
function getWinners(array,getFinalsCB) {
    return getFinalsCB(array).map((teams) => teams['Home Team Goals'] > teams['Away Team Goals'] ?
                                  teams['Home Team Name'] : teams['Away Team Name'])
  }
  
  // condition to be met ? value if true : value if false  
...
function getWinnersByYear(array, getFinalsCB, getYearsCB, getWinnersCB) {
    ...
    return winners.map((teams,index) => `In ${years[index]}, ${teams} won the world cup!`);
    }
